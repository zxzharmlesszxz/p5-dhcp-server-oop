...
my $lease_ack     = "UPDATE `ips` SET `lease_time` = UNIX_TIMESTAMP()+3600 WHERE `mac` = '%s' AND `ip` = '%s';"; #done
my $lease_offered = "UPDATE `ips` SET `lease_time` = UNIX_TIMESTAMP()+30, `mac` = '%s' WHERE `ip` = '%s';"; #done
