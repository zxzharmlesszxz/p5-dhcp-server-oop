...
my $get_routing = "SELECT `destination`, `mask` `gateway` FROM `subnets_routes` WHERE `subnet_id` = '?' LIMIT 30;";
...
my $lease_release = "UPDATE `ips` SET `lease_time` = '', `mac` = NULL WHERE `mac` ='%s';";
my $lease_success = "UPDATE `ips` SET `lease_time` = UNIX_TIMESTAMP()+3600, `mac` ='%s' WHERE `ip` = (SELECT `ip` FROM `clients` WHERE `mac` = '%s' AND`subnet_id` = (SELECT `subnet_id` FROM `subnets` WHERE `vlan_id` = '%s' AND `type` != 'guest'));";
